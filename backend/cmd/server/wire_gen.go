...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	growthService := service.NewGrowthService(client)
	userHandler := handler.NewUserHandler(userService, authService, emailService, emailCache, affiliateService, growthService, serviceUserPlatformQuotaRepository)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, growthHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	tgShopService := service.NewTGShopService(userRepository, redeemService, affiliateService, paymentService)
	tgShopWebhookHandler := handler.NewTGShopWebhookHandler(tgShopService, configConfig)
	...
	dramaVideoTaskRepository := repository.NewDramaVideoRepository(db)
	dramaVideoClient := repository.NewDramaVideoClient(configConfig)
	dramaVideoService := service.NewDramaVideoService(dramaVideoTaskRepository, accountRepository, dramaVideoClient, billingService, usageBillingRepository, usageLogRepository, userGroupRateRepository, apiKeyService, apiKeyService, modelPricingResolver)
	dramaVideoHandler := handler.NewDramaVideoHandler(dramaVideoService)
	seedanceVideoService := service.NewSeedanceVideoService(accountRepository, dramaVideoClient, dramaVideoTaskRepository, gatewayService, subscriptionService, usageBillingRepository, apiKeyService, apiKeyService)
	seedanceVideoHandler := handler.NewSeedanceVideoHandler(seedanceVideoService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, tgShopWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, dramaVideoHandler, seedanceVideoHandler, idempotencyCoordinator, idempotencyCleanupService)
