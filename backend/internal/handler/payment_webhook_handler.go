...
// DulupayNotify handles Dulupay payment notifications.
// GET/POST /api/v1/payment/webhook/dulupay
func (h *PaymentWebhookHandler) DulupayNotify(c *gin.Context) {
	h.handleNotify(c, payment.TypeDulupay)
}
...
func extractOutTradeNo(rawBody, providerKey string) string {
	switch providerKey {
	case payment.TypeEasyPay, payment.TypeDulupay, payment.TypeAlipay:
