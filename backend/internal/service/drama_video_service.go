...
func NewDramaVideoService(
	...
	var quota APIKeyQuotaUpdater
	var cache apiKeyAuthCacheInvalidator
...
func (s *DramaVideoService) Create(ctx context.Context, apiKey *APIKey, rawBody []byte, inboundPath string) (*DramaVideoCreateResult, error) {
	...
	runner(func() {
		s.processTask(context.Background(), taskID, apiKey, plan)
	})
...
func (s *DramaVideoService) prepareCreate(ctx context.Context, apiKey *APIKey, rawBody []byte, inboundPath string) (*dramaVideoCreatePlan, error) {
	if len(rawBody) == 0 {
		return nil, infraerrors.BadRequest("DRAMA_VIDEO_INVALID_REQUEST", "request body is empty")
	}
	surface := dramaVideoSurfaceVideos
	if strings.Contains(inboundPath, "/video/generations") {
		surface = dramaVideoSurfaceGens
	}
	...
	requestedModel := resolved.Family
	upstreamModel := resolved.UpstreamModel
	if mapping := account.GetModelMapping(); len(mapping) > 0 {
		if mapped, ok := mapping[requestedModel]; ok && strings.TrimSpace(mapped) != "" {
			upstreamModel = strings.TrimSpace(mapped)
		} else if mapped, ok := mapping[resolved.UpstreamModel]; ok && strings.TrimSpace(mapped) != "" {
			upstreamModel = strings.TrimSpace(mapped)
		}
	}
	body, err := marshalDramaVideoUpstreamBody(payload, upstreamModel)
	if err != nil {
	...
	if s.resolver != nil && apiKey.Group != nil {
		gid := apiKey.Group.ID
		resolvedPrice := s.resolver.Resolve(ctx, PricingInput{
			Model:   requestedModel,
			GroupID: &gid,
			Group:   apiKey.Group,
		})
		if resolvedPrice != nil && resolvedPrice.Source == PricingSourceChannel &&
			(resolvedPrice.Mode == BillingModePerRequest || resolvedPrice.Mode == BillingModeImage || resolvedPrice.Mode == BillingModeVideo) {
			units := float64(1)
			if resolved.BillingUnit == DramaVideoBillingPerSecond || resolvedPrice.Mode == BillingModeVideo && resolved.BillingUnit != DramaVideoBillingPerClip {
	...
	if cost == nil {
		cost = s.billing.CalculateDramaVideoCost(resolved, seconds, apiKey.Group.VideoPriceConfig(), videoRate)
	}
	...
	return &dramaVideoCreatePlan{
		...
		createPath:      resolved.CreatePath,
...
func (s *DramaVideoService) processTask(ctx context.Context, taskID string, apiKey *APIKey, plan *dramaVideoCreatePlan) {
	if s == nil || plan == nil || apiKey == nil {
		return
	}
	now := time.Now()
	inProgress := DramaVideoStatusInProgress
	_, _ = s.tasks.UpdateStatus(ctx, DramaVideoTaskStatusUpdate{TaskID: taskID, Status: inProgress, SubmittedAt: &now})
	upstreamTask, err := s.client.CreateVideo(ctx, &plan.account, plan.createPath, plan.body)
	if err != nil {
	...
	for {
		...
		if current != nil {
			...
			case DramaVideoStatusFailed, DramaVideoStatusCanceled:
				completedAt := time.Now()
				failedStatus := NormalizeDramaUpstreamStatus(current.Status)
				_, _ = s.tasks.UpdateStatus(ctx, DramaVideoTaskStatusUpdate{TaskID: taskID, Status: failedStatus, Progress: dramaPtrInt(clampDramaProgress(current.Progress)), Error: current.Error, CompletedAt: &completedAt})
		...
		polled, err := s.client.GetVideo(ctx, &plan.account, plan.createPath, upstreamID)
		if err != nil {
	...
	_, _ = s.tasks.UpdateStatus(ctx, DramaVideoTaskStatusUpdate{TaskID: taskID, Status: DramaVideoStatusInProgress})
	...
	if err := s.recordUsage(ctx, apiKey, plan, taskID); err != nil {
...
func (s *DramaVideoService) recordUsage(ctx context.Context, apiKey *APIKey, plan *dramaVideoCreatePlan, taskID string) error {
	...
	log := &UsageLog{
		...
		UpstreamModel:         optionalTrimmedStringPtr(plan.upstreamModel),
		GroupID:               groupID,
		...
		InboundEndpoint:       optionalTrimmedStringPtr(plan.createPath),
		UpstreamEndpoint:      optionalTrimmedStringPtr(plan.createPath),
		CreatedAt:             time.Now(),
	}
...
func DramaVideoErrorJSON(errorType, message string) json.RawMessage {
	raw, err := json.Marshal(map[string]string{"type": errorType, "message": message})
	if err != nil {
		return json.RawMessage(`{"type":"error","message":"unknown error"}`)
	}
	return raw
}
