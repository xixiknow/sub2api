...
func (s *AuthService) RegisterWithVerification(ctx context.Context, email, password, verifyCode, promoCode, invitationCode, affiliateCode string) (string, *User, error) {
	...
	if registrationInvite != nil && registrationInvite.CodeType == RegistrationCodeTypeRedeem {
		invitationRedeemCode = registrationInvite.RedeemCode
	}

	if registrationInvite != nil && registrationInvite.CodeType == RegistrationCodeTypeAffiliate && s.entClient != nil {
		...
		if err := s.applyRegistrationInvitation(txCtx, registrationInvite, user.ID); err != nil {
			return "", nil, err
		}
	...
	} else if err := s.createUserAndClaimInvitation(ctx, user, invitationRedeemCode); err != nil {
	...
	} else if registrationInvite != nil && registrationInvite.CodeType == RegistrationCodeTypeAffiliate {
	...
	if s.affiliateService != nil {
		...
		if code := strings.TrimSpace(affiliateCode); code != "" && (registrationInvite == nil || registrationInvite.CodeType != RegistrationCodeTypeAffiliate) {
	...
	// 兑换码邀请占用已由 createUserAndClaimInvitation 在“用户创建 + 邀请码占用”的
	// 同一个数据库事务内原子完成（一次性约束，见函数注释）。
	// 返佣注册席位由 resolveRegistrationInvitation + applyRegistrationInvitation 消耗。
...
func (s *AuthService) loginOrRegisterOAuthWithTokenPair(ctx context.Context, email, username, invitationCode, affiliateCode, promoCode, signupSource string) (*TokenPair, *User, error) {
	...
	if err != nil {
		if errors.Is(err, ErrUserNotFound) {
			...
			registrationInvite, err := s.resolveRegistrationInvitation(ctx, invitationCode, affiliateCode, ErrOAuthInvitationRequired)
			if err != nil {
				return nil, nil, err
			}
			...
			if s.entClient != nil && registrationInvite != nil {
				tx, err := s.entClient.Tx(ctx)
				if err != nil {
				...
				if err := s.userRepo.Create(txCtx, newUser); err != nil {
					...
					if err := s.applyRegistrationInvitation(txCtx, registrationInvite, newUser.ID); err != nil {
						return nil, nil, err
					}
					if err := tx.Commit(); err != nil {
					...
					// snapshot user × platform quota（fail-open）
					_ = s.snapshotPlatformQuotaDefaults(ctx, user.ID, &grantPlan)
					if registrationInvite.CodeType != RegistrationCodeTypeAffiliate {
