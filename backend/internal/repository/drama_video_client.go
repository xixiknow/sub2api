...
func (c *dramaVideoClient) CreateVideo(ctx context.Context, account *service.Account, body []byte) (*service.DramaVideoUpstreamTask, error) {
	if len(body) == 0 {
...
func (c *dramaVideoClient) DownloadVideo(ctx context.Context, account *service.Account, taskID string) (*service.DramaVideoDownload, error) {
	path := "/v1/videos/" + url.PathEscape(strings.TrimSpace(taskID)) + "/content"
	req, err := c.newRequest(ctx, account, http.MethodGet, path, nil)
	if err != nil {
		return nil, err
	}
